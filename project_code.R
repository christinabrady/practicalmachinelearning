...
clean <- function(dat){
  ...
  usersplit <-split(nonas, nonas$user_name) 
  timeremoved <- do.call(rbind, lapply(usersplit, function(x) return(notime(x))))
  return(timeremoved[, !(colnames(timeremoved) %in% elim_name)])
}
...
preda <- predict(rf1.1, newdata = cleantesting)
confusionMatrix(preda, cleantesting$classe)
...
confusionMatrix(predict(rf3, cleantesting), cleantesting$classe)
